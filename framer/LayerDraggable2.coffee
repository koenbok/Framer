...
class exports.LayerDraggable extends BaseClass
	...
	@define "constraints",
		get: -> @_constraints
		set: (@_constraints) ->
			@_updateInertialScrollConstraints value if value

	@define "velocity",
		get: ->
			{x: 0, y: 0}
		set: ->
			throw Error "You can't set velocity on a draggable"

	@define "angle"
		get: -> 0
		set: ->
			throw Error "You can't set angle on a draggable"
	...
	constructor: (@layer) ->
		...
		defaults = Defaults.getDefaults "LayerDraggable", {}
