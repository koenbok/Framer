let filterFormat = (value, unit) => `${Utils.round(value, 2)}${unit}`;
	// "#{value}#{unit}"

let roundToZero = function(num) {
...
export let LayerStyle = {
	...
	webkitFilter(layer) {
		...
		for (let [cssName, layerName, fallback, unit] of Array.from(_WebkitProperties)) {
...
`;
	...
	webkitTransformForce2d(layer) {
		...
		for (let p in _Force2DProperties) {
			let v = _Force2DProperties[p];
