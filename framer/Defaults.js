...
export let Defaults = {
	...
	getDefaults(className, options) {
		...
		let v;
		if (!Originals.hasOwnProperty(className)) { return {}; }
		if (!Framer.Defaults.hasOwnProperty(className)) { return {}; }

		options = _.clone(options);

		// Always start with the originals
		let defaults = _.cloneDeep(Originals[className]);
		// Copy over the user defined options
		for (var k in Framer.Defaults[className]) {
			// Make an exception for curve functions
			v = Framer.Defaults[className][k];
			if (_.isFunction(v) && !((className === "Animation") && (k === "curve"))) {
				defaults[k] = v();
			} else {
				defaults[k] = v;
			}
	...
	setup() {
		...
		if (window.FramerDefaults) {
			for (let className in window.FramerDefaults) {
				let classValues = window.FramerDefaults[className];
				for (let k in classValues) {
					let v = classValues[k];
